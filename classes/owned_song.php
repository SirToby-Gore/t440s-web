...
class OwnedSong {
    ...
    public static function is_id_in_use(string $id): bool {
        ...
        $stmt = $db->prepare("SELECT * FROM `OwnedSong` WHERE `ownership_id`=?");
    ...
    public static function from_id(string $id): self {
        ...
        if (!self::is_id_in_use($id)) {
            die("No OwnedSong found with id $id");
        }
    ...
    public function create(): void {
        ...
        $stmt = $db->prepare("INSERT INTO `OwnedSong` (`ownership_id`, `user_id`, `song_id`, `acquisition_date`) VALUES (?, ?, ?, ?)");
        $stmt->bind_param(
            'ssss',
            $this->ownership_id,
            $this->user_id,
    ...
    public function update(): void {
        ...
        $stmt = $db->prepare("UPDATE `OwnedSong` SET `user_id`=?, `song_id`=?, `acquisition_date`=? WHERE `ownership_id`=?");
        $stmt->bind_param(
            'ssss',
            $this->user_id,
            $this->song_id,
    ...
    public function delete(): void {
        ...
        $stmt = $db->prepare("DELETE FROM `OwnedSong` WHERE `ownership_id`=?");
