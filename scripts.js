...
window.initializeTabs = function () {
    ...
    tabContainers.forEach(container => {
        let initialActiveButton = container.querySelector('.tab-button.active');
        if (!initialActiveButton) {
            initialActiveButton = container.querySelector('.tab-button');
        }
        if (initialActiveButton) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const audio = document.getElementById('play-bar-audio');
    const source = document.getElementById('play-bar-source');
    const artImg = document.getElementById('play-bar-art');
    const titleSpan = document.getElementById('play-bar-title');
    const artistLink = document.getElementById('play-bar-artist-link');
    const albumLinkSpan = document.getElementById('play-bar-album-span');
    ...
    let currentQueueIndex = JSON.parse(localStorage.getItem('queue-index') ?? '-1');
    let currentPlayingSong = JSON.parse(localStorage.getItem('current-index') ?? 'null');
    // NEW: Load saved time position from localStorage
    const savedProgress = JSON.parse(localStorage.getItem('playback-progress') ?? 'null');
    // --- Utility Functions ---
    /**
    ...
    function getQueueFromCookie() {
        const dataElement = document.getElementById('play-queue-data');
    ...
    function updatePlayBarUI(songData) {
        if (artImg)
            artImg.src = songData['cover-image-path'] || 'placeholder.png';
        if (titleSpan)
            titleSpan.textContent = songData.title || 'Unknown Song';
        if (artistLink) {
    ...
    if (currentPlayingSong) {
        ...
        updatePlayBarUI({ 'title': 'Melody Vault', 'artist-name': 'Welcome', 'cover-image-path': 'placeholder.png' });
    }
    ...
    document.querySelectorAll('#shuffle-all-button').forEach(button => {
        button.addEventListener('click', () => {
            // Correctly read the page-specific queue from the hidden element
            playQueue = shuffleArray(getQueueFromCookie());
            currentQueueIndex = 0;
            if (playQueue.length > 0) {
                play(playQueue[currentQueueIndex]);
            }
    ...
    document.querySelectorAll('.song-item').forEach(item => {
        item.addEventListener('click', () => {
            const clickedSongId = item.dataset.songId;
            if (!clickedSongId)
                return;
